...
const Login:FC<Props> = ({checkLoginStatus}) => {
  ...
  const login = () => {
    ...
    LoginService.loginVerify({email:email,verificationCode:verificationCode}).then(res=>{
      if(res.data.body.success==true){
  ...
  return (
    <div className={"login-layout"}>
      <Alert message="Arex 浏览器扩展用于 Web 版接口调试，须使用 Chrome 浏览器，请下载至本地安装。" type="info" showIcon />
      <div className={"login"}>
        <div className={"login-title"}>AreX</div>
        <Input size="large" placeholder="请输入邮箱号！" prefix={<UserOutlined />} onChange={getEmail} status={emailchecked?'':'error'} allowClear/>
        {emailchecked?<div className={"login-email-tip"}></div>:<div className={"login-email-tip"}>请输入正确的邮箱号!</div>}
        <div className={"login-verificationCode"}>
          <Input size="large" placeholder="请输入验证码！" prefix={<LockOutlined />} onChange={getVerificationCode} />
          <Button size="large" onClick={sendVerificationCode} disabled={loadings}>{loadings?count+'s':''}获取验证码</Button>
        </div>
