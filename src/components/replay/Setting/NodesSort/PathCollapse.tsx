...
const PathCollapse: FC<PathCollapseProps> = (props) => {
  const handleEdit = (sortNode: SortNode) =>
    props.onEdit && props.onEdit(sortNode.path, sortNode);

  /**
   * 删除 IgnoreNode
   */
  const { run: deleteIgnoreNode } = useRequest(
    AppSettingService.deleteSortNode,
    {
      manual: true,
      onSuccess(success) {
        if (success) {
    ...
    <CollapseWrapper>
      ...
      <Spin spinning={props.loading || false}>
        <Collapse
          ...
          onChange={(id) =>
            props.onChange &&
            props.onChange(props.interfaces.find((i) => i.id === id))
          }
        >
                      ...
                      <SpaceBetweenWrapper width={"100%"}>
                        ...
                        <span>
                          ...
                          <Button
                            ...
                            icon={<DeleteOutlined />}
                            onClick={() =>
                              deleteIgnoreNode({ id: sortNode.id })
                            }
                          />
                        </span>
                      </SpaceBetweenWrapper>
