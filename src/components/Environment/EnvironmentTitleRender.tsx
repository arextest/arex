...
function EnvironmentTitleRender({val,titleRadius,updateDirectorytreeData,nowEnvironment,setNowEnvironment}: Environmentprops) {
...
const menu = (val: any) => {
  return (
    ...
    items={[
        ...
        key: '5',
        label: (
            <a style={{ color: 'red' }}  onClick={()=>showModal('delete')}>Delete</a>
        ),
        },
    ]}
    />
  ...
  return (
    ...
    `}>   
      ...
      <Modal title={`Delete "${val.envName}"`} okText='Delete' visible={isModalVisible}  onOk={()=>handleOk('delete')} onCancel={handleCancel}>
        <p>Deleting this environment might cause any monitors or mock servers using it to stop functioning properly. Are you sure you want to continue?</p>
      </Modal>
      <Modal title='rename' visible={isRenameVisible}  onOk={()=>handleOk('rename')} onCancel={handleCancel}>
