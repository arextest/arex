...
const CreateAndUpdateFolder: React.FC<any> = ({ updateDirectoryTreeData }) => {
  ...
  const collectionCreateAndUpdateModalVisible = useStore(
    (state) => state.collectionCreateAndUpdateModalVisible
  );
  const collectionCreateAndUpdateModalFolderName = useStore(
    (state) => state.collectionCreateAndUpdateModalFolderName
  );
  const collectionCreateAndUpdateModalMode = useStore(
    (state) => state.collectionCreateAndUpdateModalMode
  );
  const collectionCreateAndUpdateModalId = useStore(
    (state) => state.collectionCreateAndUpdateModalId
  );
