...
const useCheckChrome = () => {
  ...
  useEffect(() => {
    if (getChromeVersion() < 0) {
      ...
      message.info(
        <div>
          Be careful：The Chrome Extension can break the cross-domain limit of
          the browser. Please install the&nbsp;
          <a
      ...
      console.info(
        t("http.extensionIncorrect", ` ${window.__AREX_EXTENSION_VERSION__}`)
      );
    } else {
      console.info(
        `[AREX] Extension installed, version ${window.__AREX_EXTENSION_VERSION__}`
      );
    }
  }, []);
};
