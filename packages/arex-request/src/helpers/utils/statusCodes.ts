const statusCodes: {
  ...
  405: 'Method Not Allowed', // A request was made of a resource using a request method not supported by that resource;[2] for example, using GET on a form which requires types to be presented via POST, or using PUT on a read-only resource.
  ...
  415: 'Unsupported Media Type', // The request entity has a media types which the server or resource does not support.[2] For example, the client uploads an image as image/svg+xml, but the server requires that images use a different format.
