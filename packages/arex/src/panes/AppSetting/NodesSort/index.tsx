...
const SettingNodesSort = forwardRef<SettingNodesSortRef, SettingNodesSortProps>((props, ref) => {
  ...
  const {
    ...
    (listPath?: string[]) =>
      props.interfaceId
        ? ComparisonService.queryInterfaceSortNode({
   ...
   * 更新 SortNode
   */
  const { run: updateSortNode } = useRequest(ComparisonService.updateSortNode, {
    manual: true,
  ...
  const { run: updateInterfaceResponse } = useRequest(
    (params) =>
      props.interfaceId
        ...
        : ConfigService.updateInterfaceResponse(params),
    {
      manual: true,
      onSuccess(success) {
        if (success) {
